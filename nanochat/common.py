...
def download_file_with_lock(url, filename, postprocess_fn=None):
  ...
  with FileLock(lock_path):
    ...
    # All other ranks are blocked until it is released
    ...
    # Run the post process function if provided
...
def compute_init(device_type='cuda'):
  '''Basic repetitive initialization'''
  assert device_type in ['cuda', 'mps', 'cpu'], 'Invalid device type'

  if device_type == 'cuda':
    assert torch.cuda.is_available(), 'This pytorch installation is not configured for CUDA but device type is "cuda"'
  
  if device_type == 'mps':
    assert torch.mps.is_available(), 'This pytorch installation is not configured for MPS but device type is "mps"'

  # Reproducibility
  # We set global seeds here, but mode of the code uses explicit rng objects
...
def compute_cleanup():
  '''Companion of compute_init, to clean things up before process exit'''
