...
FlashAttention-3 (FA3) combined with Scaled Dot-Product Attention (SDPA) represents the cutting-edge of GPU-accelerated attention mechanisms, 
specifically optimized for NVIDIA Hopper architecture (H100/H200 GPUs) to significantly speed up LLM training and inference. 
...
def _load_flash_attention_3():
  ...
  try:
    major, _ = torch.cuda.get_device_capability() # the major and minor cuda capability of the device
...
def _sdpa_attention(q, k, v, window_size, enable_gqa):
  ...
  row_idx = (Tk - Tq) + torch.arange(Tq, device=device).unsqueeze(0)
