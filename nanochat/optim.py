...
def adamw_step_fused(
    ...
    )  # each element of exp_avg/denom is scaled by alpha before adding
...
def muon_step_fused(
    ...
    if g.size(-2) > g.size(-1):  # Tall matrix
        ...
        for (
            ...
            b,
            c,
        ) in polar_express_coeffs[:ns_steps]:
            # This is a third-order polynomial update.
            # By including the $A^2$ term (the square of the covariance),
            # the algorithm can correct errors much more aggressively than a linear update. It's essentially 'Newton's Method on steroids' for matrices.
            A = X @ X.mT
...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    def _compute_adamw(
        ...
        for p in group["params"]:
            ...
            # large params need to gather
            if not pinfo["is_small"]:
                future = dist.all_gather(p, p_slice, async_op=True).get_future()
    ...
    def step(self):
        ...
        # Phase 3: wait for gathers and then copy back
